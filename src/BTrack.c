...
int btrack_init(struct btrack * bt, int hop_size, int frame_size, int sample_rate){
	...
	for (int i = 0;i < BTRACK_ARRAY_SIZE(bt->prevDelta);i++) {
		bt->prevDelta[i] = 1;
	}

//	double t_mu = BTRACK_ARRAY_SIZE(bt->delta)/2;
	double m_sig;
	double x;
	// create tempo transition matrix
	m_sig = 41/8;
	for (int i = 0;i < BTRACK_ARRAY_SIZE(bt->tempoTransitionMatrix);i++) {
		for (int j = 0;j < BTRACK_ARRAY_SIZE(bt->tempoTransitionMatrix[0]);j++) {
			x = fabs(j+1-i);
//			t_mu = i+1;
			bt->tempoTransitionMatrix[i][j] = (1 / (m_sig * sqrt(2*pi))) * exp( (-1*pow((x),2)) / (2*pow(m_sig,2)) );
		}
	}
...
static void calculateTempo(struct btrack * bt){
	...
	for (int i = 0;i < BTRACK_ARRAY_SIZE(bt->tempoObservationVector);i++) {
	...
	if(bt->tempoFixed) {
		for (int k = 0;k < BTRACK_ARRAY_SIZE(bt->prevDelta);k++) {
			bt->prevDelta[k] = bt->prevDeltaFixed[k];
		}
	}

	for(int j=0;j < BTRACK_ARRAY_SIZE(bt->tempoTransitionMatrix);j++) {
		maxval = -1;
		for (int i = 0;i < BTRACK_ARRAY_SIZE(bt->tempoTransitionMatrix[0]);i++) {
	...
	normaliseArray(bt->delta,BTRACK_ARRAY_SIZE(bt->delta));

	maxind = -1;
	maxval = -1;

	for (int j=0;j < BTRACK_ARRAY_SIZE(bt->delta);j++) {
	...
	for (int i = 2;i <= 127;i++) {
		...
		for (int a = 1;a <= numelem;a++) {
			...
			for (int b = 1-a;b <= a-1;b++) {
				bt->combFilterBankOutput[i-1] += (bt->acf[(a*i+b)-1]*bt->weightingVector[i-1])/(2*a-1);	// calculate value for comb filter row
			}
		}
	}
}
